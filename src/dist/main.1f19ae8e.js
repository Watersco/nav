...
var $ulList = $(".ulList");
var $lastLi = $ulList.find("li.lastLi"); // const $search1 = $(".touch-search");
// const $body = $("body");

var x = localStorage.getItem("x"); //此时取出 x 还是一个对象

var xObject = JSON.parse(x); //把字符串变成对象 parse不能访问空字符串
//创建哈希数组表

var hashMap = xObject || [//xObject存在就用x，否则就用初始↓↓
{
  logo: "B",
...
var removeX = function removeX(url) {
  return url.replace("https://", "").replace("http://", "").replace("www.", "").replace(/\/.*/, ""); // \是为了转意 删除 / 开头的内容
};
...
window.onbeforeunload = function () {
  ...
  localStorage.setItem("x", string); // 在本地存储设置并命名为x，x就是string;
};
